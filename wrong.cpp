...
void first_max_Occurence(vector<int> arr, int n) {
    ...
    // Determine which elements are the maximum occurrences
    for (auto i : map) {
        if (i.second > max) {
            max = i.second;
            vec2.clear();
    ...
    int ele = -1;  // Initialize to a default value (make it type based)

    // Find the first index of the maximum occurring element
    ...
    if (ele != -1) {  // Check if a valid element was found
